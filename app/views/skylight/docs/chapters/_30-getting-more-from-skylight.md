...
<%= render layout: "note", locals: { type: "important" } do %>
  About those query names: Remember that Skylight works best when aggregating trace data, so to keep your dynamically-named GraphQL queries appropriately grouped, it's important to limit the set of all possible names (we think 100 or so is a reasonable number).
<% end %>
