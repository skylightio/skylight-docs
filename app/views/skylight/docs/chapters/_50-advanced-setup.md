...
You can also check the required Ruby version based on <%= link_to "the specific version", "https://rubygems.org/gems/skylight" %> of the Skylight agent you are running. For the curious, our CI currently is testing against <%= link_to "these versions", "https://github.com/skylightio/skylight-ruby/blob/main/.github/workflows/build.yml" %>. Intermediate versions are expected to work as well.
